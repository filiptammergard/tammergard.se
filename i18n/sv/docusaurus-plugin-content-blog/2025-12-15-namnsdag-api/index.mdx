...
Alla endpoints ligger under `/api/v1/`, returnerar JSON och har CORS aktiverat för alla domäner. Statiska endpoints (`/all`, `/on`, `/when`, `/where`, `/who`) CDN-cachas i en vecka, medan `/today` cachas fram till nästa midnatt i `Europe/Stockholm` så att gårdagens namn inte ligger kvar efter midnatt. Fullständig dokumentation på [namnsdag.tammergard.se/api](https://namnsdag.tammergard.se/api).
