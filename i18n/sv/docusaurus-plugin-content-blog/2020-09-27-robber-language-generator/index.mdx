...
> Vi tar en mening, expanderar varje x och konverterar resultatet till rövarspråk.
...
Eller så kompakterar vi exempel 10 genom att fälla in x-fallet direkt i `consonantToRobber`:
...
Teckenklassen med flaggan `i` matchar konsonanter i både versal och gemen form, och callback-funktionen hanterar versaler genom att göra den efterföljande konsonanten gemen. x hanteras separat (i det första `replaceAll`-anropet) eftersom versalt X ska expandera till blandad-case "Ks" – något en enkel teckenklass-ersättning inte kan producera i ett steg, vilket är varför förbehandlingssteget från tidigare i posten fortfarande behövs.
