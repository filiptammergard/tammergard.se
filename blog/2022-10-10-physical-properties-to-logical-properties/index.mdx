...
description: Swap absolute CSS properties like margin-left for writing-mode-relative logical properties so layouts adapt to right-to-left and vertical scripts.
authors: [filip]
tags: [css]
---
...
Let's say we want to display a quote with a red border to the left together with some padding:
...
But what happens if we translate the paragraph to Arabic? Arabic has a right-to-left text direction, so let's specify that with `dir="rtl"`, and mark that Arabic is used in the paragraph with `lang="ar"`. Let's try updating the text direction while keeping the same CSS:
...
The margin property also has shorthands equivalent to the padding shorthands.

<details>
	<summary>
		Proposal for flow-relative <code>margin</code> shorthand
	</summary>
	In the future, we might have a <code>margin-logical</code>, as a logical
	shorthand equivalent to the physical <code>margin</code> shorthand. See the
...
Browser support for logical properties is generally great.
...
Does it ring a bell? It probably should. In this declaration, the logical `inline-size` property is used, but the declaration value `70vw` is physical. In other words, `inline-size` can define the horizontal or vertical size depending on writing mode, while `70vw` is always calculated based on the horizontal size independent of the writing mode. In practice, we probably want the value to be flow relative as well in this case.

The units `vb` and `vi` can make this happen! `vb` is the percentage of the viewport's block size, while `vi` is the percentage of the viewport's inline size. So here's what you can do instead to make the value flow relative:
