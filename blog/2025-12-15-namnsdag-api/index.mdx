...
Behind the API sits a small site at [namnsdag.tammergard.se](https://namnsdag.tammergard.se). You can see today's namedays, look up when a name has its day, browse the year month by month, switch between Sweden, Norway, Denmark, Germany, Czechia, and Poland, and open a dedicated page for each name (`/namn/{name}`) and each date (`/datum/{date}`).
