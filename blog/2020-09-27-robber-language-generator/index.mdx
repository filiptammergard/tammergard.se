...
const robberSentence = toRobber(expandX(sentence))
...
> We take a sentence, expand each x, and convert the result to robber language.
...
The character class with the `i` flag matches consonants in either case, and the callback handles uppercase by lowercasing the trailing consonant. The "x" is handled separately (in the first `replaceAll`) because uppercase "X" needs to expand to mixed-case "Ks" — something a single character-class substitution can't produce in one step, which is why the preprocessing step from earlier in the post is still needed.
