...
description: Why border-radius 50% produces an ellipse, and how the border-radius longhand properties let you draw far more interesting curved shapes.
authors: [filip]
tags: [css]
---
...
So what does a value in percentages actually mean when it comes to `border-radius`? In this example, `50%` makes the circle radius in the horizontal axis equal 50% of the width, and `25%` makes the circle radius equal 25% of the height.
...
Now we have to change the `border-radius` value to `100px` to make it a circle again. Instead of doing the math ourselves when we want to achieve a circle, we can let CSS do the math by using `border-radius: 50%`!
...
- The individual `border-radius` longhand properties allow two values, to control the circle radius in the two axes individually.
- When using percentages, the circle radius relates to the width in the horizontal axis and the height in the vertical axis, which makes achieving ellipses simple.
