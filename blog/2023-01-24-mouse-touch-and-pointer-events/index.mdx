...
description: How mouse, touch and pointer events evolved on the web, why compatibility mouse events cause ambiguity, and when to listen to each type.
authors: [filip]
tags: [javascript]
---
...
> If the preventDefault method of touchstart or touchmove is called, the user agent should not dispatch any mouse event that would be a consequential result of the prevented touch event.
...
Mouse and touch events are still used in many places, probably partly due to the fact that they have better historical browser support. But pointer events are solidly supported across modern browsers and most of the web would probably benefit from using pointer events only.
