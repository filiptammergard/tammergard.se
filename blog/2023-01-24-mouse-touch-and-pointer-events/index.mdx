...
Historically, the mouse was the only type of input device. Because of that, the only type of input event was [mouse events](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent). This is the baseline.

Then came touch devices. Touching a screen with a finger can partly be represented with a mouse click, but for example when it comes to multi-touch, such representation is not possible. We needed touch events!
...
- When no `preventDefault` is called, clicking the button with a finger on a touch device results in one `touchstart` event and one `pointerdown` event.
...
- When `preventDefault` is called on the `pointerdown` event, no `mousedown` event is fired when clicking the button with a mouse.
