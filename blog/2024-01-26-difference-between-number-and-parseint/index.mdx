...
description: Why Number() and parseInt() disagree on inputs like "3px", booleans and empty strings, and which one to reach for when parsing numbers.
authors: [filip]
tags: [javascript]
---
...
- `Number()` can return any number primitive, while `parseInt()` returns integers.
...
There's one fundamental difference between the two that is source to many of these differences:
...
`[].toString` becomes an empty string, which means `parseInt([])` becomes `NaN`. On the other hand, `[3].toString()` becomes `"3"`, so that `parseInt([3])` becomes `3`.
...
Since `"3 hello"` and `"3_300"` begin with something parsable `"3"`, `parseInt()` will use that, while `Number()` will return `NaN`.
